import {
  FinalInspectionDetail as ErpFinalInspectionDetail,
  FinalInspectionDocument as ErpFinalInspectionDocument,
} from '@/types/erp-db.generated'
import { FinalInspection, type FinalInspectionId } from '@/domain/quality/fqc/entities/FinalInspection'
...
export type FinalInspectionLoadModel = {
   ...
   *
   */
  readonly fallbackId: FinalInspectionId
}
...
export type FinalInspectionPersistenceModel = {
   ...
   *
   */
  readonly document: ErpFinalInspectionDocument
  /**
   *
   * 明细 DTO 集合。
   *
   */
  readonly details: ErpFinalInspectionDetail[]
}
 ...
 *
 */
  public static toDomain(model: FinalInspectionLoadModel): FinalInspection | null {
   ...
   *
   */
  public static toPersistence(entity: FinalInspection): FinalInspectionPersistenceModel {
    const doc = new ErpFinalInspectionDocument()
    ...
    const details = entity.details.map((detail) => {
      const dto = new ErpFinalInspectionDetail()
   ...
   *
   */
  private static mapDetails(details: readonly unknown[]): FinalInspectionDetail[] {
    const usedIds = new Set<number>()
    let nextTempId = -1

    const result: FinalInspectionDetail[] = []
