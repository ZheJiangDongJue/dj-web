...
describe('FinalInspectionRepositoryImpl', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
  })
  ...
  it('findById: 调用 GetBillWithDetails 并映射聚合', async () => {
    ...
    const { FinalInspectionRepositoryImpl } = await import('./FinalInspectionRepositoryImpl')
    ...
    const repo = new FinalInspectionRepositoryImpl()
    const entity = await repo.findById(10)

    expect(getBillMock).toHaveBeenCalledWith(expect.objectContaining({ tableName: 'FinalInspectionDocument', billId: 10 }))
  ...
  it('save: 映射 DTO 调用 GeneralBillSave，并在返回新 Id 时更新聚合', async () => {
    ...
    const aggregate = FinalInspection.createDraft().withId(0)
    const repo = new FinalInspectionRepositoryImpl()
    ...
    expect(payload.tableName).toBe('FinalInspectionDocument')
  ...
  it('save: 未返回新 Id 时保持原聚合', async () => {
    ...
    const aggregate = FinalInspection.createDraft().withId(7)
    const repo = new FinalInspectionRepositoryImpl()
  ...
  it('save: 支持从 data.BillId 提取新 Id', async () => {
    ...
    const repo = new FinalInspectionRepositoryImpl()
    const saved = await repo.save(FinalInspection.createDraft())
  ...
  it('delete: 兼容 isSuccess 字段与 msg 提示', async () => {
    const { GeneralBillDelete } = await import('@/lib/erp/bill-api')
    const { FinalInspectionRepositoryImpl } = await import('./FinalInspectionRepositoryImpl')

    const deleteMock = GeneralBillDelete as unknown as Mock
    deleteMock.mockResolvedValueOnce({ isSuccess: true, msg: 123 })

    const repo = new FinalInspectionRepositoryImpl()
    await expect(repo.delete(3)).resolves.toEqual({ success: true, message: '123' })
  })

  it('delete: isSuccess 为 false 时按提示返回失败', async () => {
    const { GeneralBillDelete } = await import('@/lib/erp/bill-api')
    const { FinalInspectionRepositoryImpl } = await import('./FinalInspectionRepositoryImpl')

    const deleteMock = GeneralBillDelete as unknown as Mock
    deleteMock.mockResolvedValueOnce({ isSuccess: false, Message: 'upper' })

    const repo = new FinalInspectionRepositoryImpl()
    await expect(repo.delete(4)).resolves.toEqual({ success: false, message: 'upper' })
  })
})
