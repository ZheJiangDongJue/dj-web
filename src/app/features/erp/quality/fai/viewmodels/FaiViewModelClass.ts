...
import { setLastFaiBillIdToStorage, hasStatusFlag, parseDocumentStatus } from '../../shared/helpers'
import type { DebugMenuItem } from '@/components/molecules/DebugFab'
import { FirstInspectionDocument, FirstInspectionDetail, DocumentStatus } from '@/types/erp-db.generated'
...
import {
  ...
  type FirstInspectionScanResult,
} from '@/application/quality/fai/FirstInspectionApplicationService'
...
import {
  AppServicesContext,
  FirstInspectionApplicationServiceToken,
...
export class FaiViewModel extends QualityDocumentBase<FirstInspectionDocument, FirstInspectionDetail> {
   ...
   *
   */
  public bill: FirstInspectionDocument
  /**
   *
   * 当前单据明细列表。
   *
   */
  public details: FaiDetailView[] = []
  /**
   *
   * 当前单据状态。
   *
   ...
   *
   */
  private readonly appService: FirstInspectionApplicationService
  /**
   *
   * 最近一次审批/反审批结果。
   *
   ...
   *
   */
  private static readonly docNormalizeTemplate = (() => { const doc = new FirstInspectionDocument(); doc.initDefaults(); return doc })()
  /**
   *
   * 标准化字段名的明细模板（仅取键名，不会被修改）。
   *
   */
  private static readonly detailNormalizeTemplate = (() => { const detail = new FirstInspectionDetail(); detail.initDefaults(); return detail })()

  /**
   *
   * 构造函数。
   * @param appService 首件检验应用服务实例。
   *
   */
  constructor(appService: FirstInspectionApplicationService) {
    const createEmptyDoc = () => { const doc = new FirstInspectionDocument(); doc.initDefaults(); return doc }
    ...
    // 初始化单据（使用类型默认值）
    this.bill = this.createEmptyFirstInspectionDocument()
    this.details = []
    ...
    const service: DocumentService<FirstInspectionDocument, FirstInspectionDetail> = {
    ...
    this.configure({
      ...
      onAfterRefresh: async ({ document, details }, ctx) => {
        ...
        try {
          ...
          const normDoc = this.normalizeCaseInsensitive(rawDoc, FaiViewModel.docNormalizeTemplate)
          ...
          console.error('[FAI] onAfterRefresh 更新状态失败:', err)
        }
      },
      initialId: null,
      statusApprovedValue: DocumentStatus.已审批,
 ...
 this.bill = { ...(this.bill as any), id } as FirstInspectionDocument
 this.currentId = id
 this.emit()
 }
 ...
 await this.refresh(id, { silent: true })
      ...
      console.error('[FAI] 保存后刷新单据编号失败:', err)
    }
  ...
  private redirectToNcrPrompt(billId: number): void {
    ...
    const url = new URL('/features/erp/quality/fai/ncr-prompt', window.location.origin)
    url.searchParams.set('from', 'fai')
    url.searchParams.set('action', 'approve')
    url.searchParams.set('type', 'FAI')
  ...
  public async loadInspectorOptions(): Promise<void> {
    try {
      ...
      console.error('[FAI] 加载员工列表失败:', err)
  ...
  public async loadMaterialOptions(): Promise<void> {
    try {
      ...
      /* istanbul ignore next: 加载失败仅记录日志 */
      console.error('[FAI] 加载物料列表失败:', err)
 ...
 this.emit()
      ...
      console.error('[FAI] 加载 TypeofWork 列表失败:', err)
      this.processOptions = []
      this.processIndex = {}
      this.emit()
    }
   ...
   *
   */
  private createEmptyFirstInspectionDocument(): FirstInspectionDocument {
  ...
  /**
   *
   * 构造“模拟扫码”用的首件检验草稿状态（仅用于 Debug 菜单演示）。
   * @returns 可用于 replaceState 的局部状态
   *
   */
  /* istanbul ignore next */
  private buildMockScanState(): Partial<{ bill: FirstInspectionDocument; details: FaiDetailView[]; rawDocument: any }> {
    const doc = new FirstInspectionDocument()
    doc.initDefaults()
    ...
    const d0 = new FirstInspectionDetail()
    d0.initDefaults()
    ...
    const d1 = new FirstInspectionDetail()
    d1.initDefaults()
    ...
    const details = [d0, d1]
    const billForView: any = { ...docAny, process: '首件检验' }

    return { bill: billForView as FirstInspectionDocument, details: details as any, rawDocument: docAny }
  }
  ...
  * 应用服务扫码结果处理。
  * @param result 扫码结果
  * @param scanCodeForRedeliver 本次扫码文本（用于“非激活实例”兜底转交）
  * @returns 是否已处理
  *
  */
  /* istanbul ignore next */
  private async applyScanResult(result: FirstInspectionScanResult, scanCodeForRedeliver?: string): Promise<boolean> {
    ...
    if (result.type === 'DRAFT_LOADED') {
      await this.applyFirstInspectionDraft(result.document, result.details, result.message, scanCode)
      return true
    }
  ...
  /**
   *
   * 将首件检验草稿写入 VM，并根据是否存在单据ID进行刷新与默认提示。
   ...
   */
  /* istanbul ignore next */
  private async applyFirstInspectionDraft(
    doc: FirstInspectionDocument | null,
    details: FirstInspectionDetail[],
    msg?: string,
    ...
    const normDoc = doc
      ? this.normalizeCaseInsensitive(doc, FaiViewModel.docNormalizeTemplate)
      : this.createEmptyFirstInspectionDocument()
    const normDetails = Array.isArray(details)
      ? (details as any[]).map((d) => this.normalizeCaseInsensitive(d, FaiViewModel.detailNormalizeTemplate))
      : []
    ...
    if (id > 0) {
      ...
      try { toast.success(notice || '已打开/生成首件检验单据') } catch { }
    } else {
      if (scanCode && !this.isScanListenerActive()) {
        this.redeliverScanCodeToActive(scanCode)
        return
      }
      try { toast.success(notice || '已载入首件检验数据') } catch { }
    }
  }
  ...
  */
  ...
  private async trySetInspectorByEmployeeScan(payload: { employeeId?: number | null; codeForScan?: string | null }): Promise<boolean> {
    ...
    const employeeId = FaiViewModel.toPositiveInt(payload.employeeId)
    if (employeeId) {
      if (!this.isScanListenerActive()) {
  ...
  /**
   *
   * 通过日计划明细 ID 生成/打开首件检验草稿。
   * @param dailyPlanDetailId 日计划明细主键。
   *
   */
  /* istanbul ignore next */
  private async tryOpenFirstInspectionByDailyPlanDetailId(dailyPlanDetailId: number): Promise<boolean> {
  ...
  /**
   *
   * 通过日计划明细 CodeForScan（如 RJH-...）生成/打开首件检验草稿。
   * @param codeForScan 日计划明细扫码编码。
   *
   */
  /* istanbul ignore next */
  public async tryOpenFirstInspectionByDailyPlanDetailScanCode(codeForScan: string): Promise<boolean> {
   ...
   */
  /* istanbul ignore next */
  private async handleScan(code: string): Promise<FirstInspectionScanResult> {
    try {
...
/**
 *
 * React 工厂：创建并持有 FaiViewModel 实例（仅负责生命周期，不包含业务）。
 *
 */
/* istanbul ignore next */
export function useFaiViewModelClass(): FaiViewModel {
  const container = useContext(AppServicesContext)
  const [vm] = useState(() => {
    const service =
      container?.get(FirstInspectionApplicationServiceToken) ??
      new FirstInspectionApplicationService(new FirstInspectionRepositoryImpl())
    return new FaiViewModel(service)
  })
  useEffect(() => {
