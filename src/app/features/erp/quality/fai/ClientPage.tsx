 ...
 *
 * 首件检验（客户端）View
 * - 仅负责渲染与绑定，所有流程/服务逻辑下沉至 ViewModel(useFaiViewModel)
 * @param initialScanCode 可选的 URL 透传参数：用于自动打开“日计划明细 → 首件检验草稿”
 *
 */
export default function ClientPage({ initialScanCode }: { initialScanCode?: string | null }) {
  useFeaturesPageTitle('首件检验')
  const ROW_GAP_PX = 2 as const
  return <FaiBody rowGap={ROW_GAP_PX} initialScanCode={initialScanCode} />
}
 ...
 *
 */
import { useFaiExternal } from './viewmodels/useFaiExternal'
...
function FaiBody({ rowGap, initialScanCode }: { rowGap: number; initialScanCode?: string | null }) {
  ...
  // 2) ?scancode=RJH-...：按日计划明细条码生成/打开首件检验草稿（避免与扫码入口重复逻辑）
  useEffect(() => {
    ...
    lastAutoOpenKeyRef.current = key
    void vm.tryOpenFirstInspectionByDailyPlanDetailScanCode(rawScanCode)
  }, [queryId, queryBillId, queryAction, queryScanCode, initialScanCode, vm])

  // 支持中间页“反审批”回跳：
  // - 回跳 URL: /features/erp/quality/fai?action=unapprove&billId=...
  ...
  useEffect(() => {
    ...
    if (!billId) {
      ...
      try { router.replace('/features/erp/quality/fai') } catch {}
      return
    }
    ...
    void (async () => {
      ...
      if (!opened) {
        ...
        try { router.replace(`/features/erp/quality/fai?id=${billId}`) } catch {}
        return
      }
  ...
  return (
    <>
      <FlowDetailPickDialog
        ...
        title="请选择工序明细"
        description="检测到多条“当前工序明细”，请选择要生成首件检验的工种"
 ...
 *
 */
const FaiDetailsSection = memo(function FaiDetailsSection({
  details,
  itemClassName,
  disableDetailEdit,
  disableRemoveDetail,
...
function HeaderDocument({
  ...
  return (
    <div className="w-full">
      ...
      {/* 第三行：送检数 检验员 判定 */}
      <div
        className="grid grid-cols-[auto_minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-x-0.5 gap-y-1 sm:grid-cols-[40px_46px_40px_73px_29px_93px] sm:gap-x-[2px] sm:gap-y-0"
        style={{ marginTop: rowGap }}
      >
